#!/usr/bin/env python3
"""Benchmark native full audio encoder/projector against a Torch eager reference."""
...
from bench_audio_layer0 import _make_positional, _sync, _torch_dtype
...
def _bench_cpp(encoder_bin: Path, gguf: Path, audio: Path, threads: int, repeat: int, backend: str) -> list[float]:
    times = []
    for _ in range(repeat):
        ...
        times.append(float(meta["encoder_ms"]))
    return times


def _audio_layer_forward(hidden_states, weights: dict[str, torch.Tensor], heads: int):
...
def _bench_torch(
    ...
    projector = {name: tensor.to(device=device, dtype=dtype) for name, tensor in projector.items()}
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Benchmark Qwen3-ASR full audio encoder implementations")
    ...
    cpp_results = {
        backend: _bench_cpp(encoder_bin, args.gguf, args.audio, args.threads, args.repeat, backend)
    ...
    print(f"shape=({tokens}, {projector['proj1_bias'].shape[0]})")
